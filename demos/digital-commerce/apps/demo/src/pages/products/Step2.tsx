...
const Step2 = () => {
    ...
    useEffect(() => {
        if (categoryLength === 0) {
            ...
            axios.post(`${API_BASE}/images`, {
                sessionID: sessionID,
                prompt: prompt,
                value: product.images
            } as api.ImagePromptRequest).then(resp => {
                if (resp.status === 200) {
                                    ...
                                    <CardContent>
                                        <RadioGroup onChange={handleCategoryChange}>
                                            {categories.map((c: Category, idx) => (
                                                <FormControlLabel key={`cat_sel_${idx}`} value={idx} control={<Radio/>}
                                                                  label={c.name}/>
                                            ))}
                                        </RadioGroup>
                                    </CardContent>
                                    <CardActions sx={{justifyContent: 'right'}}>
                                        <Button variant="contained" disabled={selectedCategory === -1}
                                                onClick={handleClick}>
                                            Next
                                        </Button>
                                    </CardActions>
                                                        ...
                                                        <Box>
                                                            ...
                                                            {attr.valueRange.length > 0 ? (
                                                                <>
                                                                    <br/>
                                                                    <Typography
                                                                        variant="body2">{attr.valueRange.join(', ')}</Typography>
                                                                </>
                                                            ) : (
                                                                <></>
                                                            )}
